...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    lenght=3
    volume_cube = side_cubed(lenght)
    self.assertEqual(27,volume_cube)
  ...
  def test_reverse_string(self):
    string="text"
    text_reverse = string_reverse(string)
    self.assertEqual("txet", text_reverse)

  #Given a value in farenheit, convert this into celsius.
  
  def test_fahrenheit_to_celsius(self):
    farhrenheit = 122
    celsius = fahrenheit_conversion(farhrenheit)
    self.assertEqual(50,celsius)
